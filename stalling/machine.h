	...
	stageFunction{ &Machine::IFstage , &Machine::IDstage , &Machine::EXstage, &Machine::MEMstage, &Machine::WBstage }
{
	...
	alu._operand0 = &idex.data0_;
	alu._operand1 = &alu_operator1_src.output_;
	...
	hazarddetection._exmem_reg_write = &exmem.reg_write_;
	hazarddetection._exmem_rwrite = &exmem.rwrite_;
	hazarddetection._idex_rd = &idex.rd_;
	hazarddetection._idex_mem_read = &idex.mem_read_;
	hazarddetection._idex_reg_write = &idex.reg_write_;
	...
	alu_operator1_src._input0 = &idex.data1_;
	alu_operator1_src._input1 = &idex.immed_;
	alu_operator1_src._select = &idex.alusrc_;
...
inline void Machine::assembler(const char* filename)
{
...
inline void Machine::IDstage()
{
	hazarddetection.stall_ = 0;
	ifid.run();
	hazarddetection.run();
	ifid.run();
	signextendunit.run();
...
inline void Machine::EXstage()
{
	idex.run();
	shiftleft2unit.run();
	pc_immed_adder.run();
	rwrite_src.run();
	alu_operator1_src.run();
	alu_controller.run();
	alu.run();

}

inline void Machine::MEMstage()
{
	exmem.run();
	andgate.run();
	datamemory.run();
...
{
	std::vector<std::pair<InstructionNo, State>> pipe;
	int count = 1;
	do {
		WBstage();
		MEMstage();
		...
		InstructionNo curr;
		unsigned int opcode = *(unsigned int*)&instructmemory.instruction_ >> 26;
		if (opcode == 0b100011) {
...
inline void Machine::outputPipes(std::vector<std::pair<InstructionNo, State>>& pipe, FILE* fp)
{
	for (auto it = pipe.begin(); it != pipe.end();) {
		if ((it->second == State::ID && *ifid._flush) || (it->second == State::ID && *ifid._stall) || (it->second == State::EX && *idex._flush)) {
			it->first = InstructionNo::NOP;
		}
		outputPipe(it->first, it->second, fp);
		it->second.toNext();
...
inline void Machine::outputPipe(InstructionNo inst_no, State state, FILE* fp)
{
...
inline int Machine::readInstruction(FILE* fptr)
{
...
inline int Machine::getNextInteger(FILE* fptr)
{
	int ret = 0;
	bool neg = false;
	char readch = 0;
